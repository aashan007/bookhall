...
function HallEditForms(props) {
    ...
    const{title,location,price,content,capacity,from,to} = values;
    ...
    return (
        ...
        <Select value={capacity} onChange={(value)=>setValues({...values,capacity:value})} className="w-100 m-2" size="large" placeholder="Hall Capacity">
        ...
        {from && <DatePicker defaultValue={moment(from,"DD-MM-YYYY")} placeholder="From Date" className="form-control m-2" onChange={(date,dateString) =>setValues({...values,from:dateString})}
    ...
    {/* changes needed it should dynamically check if from date is less than to date and if the from date is filled only then enable it    */}
      {to &&  <DatePicker defaultValue={moment(to,"DD-MM-YYYY")} placeholder="To Date" className="form-control m-2" onChange={(date,dateString) =>setValues({...values,to:dateString})}
        disabledDate={(current)=>current && current.valueOf() < moment().subtract(1,'days')}/>
}
